...
function recipesTemplate(recipe) {
    return `<div class="recipe">
                <img src=${recipe.image} alt=${recipe.name} width="750" height="563">
                <div class="recipe-text">
                    <div class="category">${tagTemplate(recipe.tags)}</div>
                    <h2>${recipe.name}</h2>
...
function ratingTemplate(rating) {
		let html = `<span class="rating" role="img" aria-label="Rating: 4 out of 5 stars">`
    for (let i = 1; i <= 5; i++) {
      if (i <= rating) {
